...
void process_command_line_options(int argc, char **argv)
{
    ...
    if (data_filename == NULL || model_filename == NULL || lmax < 0 || N < 0)
        usage(argv);
}
...
int main(int argc, char **argv)
{
    ...
    // double *P = malloc((lmax + 1) * (lmax + 2) * sizeof(*P));

    Matrix_d *P = Matrix_new_d(1, (lmax + 1) * (lmax + 2) / 2);
    ...
    // go ahead and store the values in a matrix
    Matrix_d *values = Matrix_new_d(p, t);
    ...
    for (;;)
    {
        ...
        /* compute error */
        computeP(&model, P->data, sin(phi));
            double vmodel = evaluate(&model, P->data, lambda);

        // printf("f_hat: %lf\n", vmodel);

        values->data[npoint] = vmodel;
